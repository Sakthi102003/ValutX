            ...
            <div className="bg-black/90 w-full max-w-lg border border-primary/20 rounded-sm shadow-[0_0_50px_rgba(0,0,0,0.8)] p-6 md:p-8 relative animate-in zoom-in-95 duration-200 overflow-hidden group my-auto">
                ...
                <div className="mb-6 md:mb-8 relative z-20">
                    <div className="flex items-center space-x-3 mb-2">
                        ...
                        <h2 className="text-[10px] md:text-xs font-black uppercase tracking-[0.4em] text-primary/70">{item.type} Archive</h2>
                    </div>
                    <h1 className="text-2xl md:text-3xl font-black uppercase tracking-tighter text-white break-all">
                        {item.data.name}
                    </h1>
                </div>
                ...
                <div className="space-y-4 md:space-y-6 relative z-20">
                    ...
                    {item.type === 'note' && (
                        <div className="space-y-2">
                            ...
                            <div className="p-4 bg-white/5 border border-white/10 rounded-sm text-xs font-mono tracking-widest text-primary/90 whitespace-pre-wrap max-h-40 md:max-h-60 overflow-auto scrollbar-hide">
                                {item.data.note}
                            </div>
                        </div>
                    )}
                ...
                <div className="mt-8 md:mt-10 pt-4 md:pt-6 border-t border-white/10 relative z-20">
                    <details className="group/details">
                        <summary className="list-none cursor-pointer flex items-center justify-between text-primary/30 hover:text-primary transition-colors">
                            <div className="flex items-center space-x-2">
                                ...
                                <span className="text-[8px] md:text-[9px] font-black uppercase tracking-[0.2em]">Provenance Telemetry</span>
                            </div>
                            <span className="text-[7px] md:text-[8px] font-mono opacity-50">NODE_VER_1.0.4</span>
                        </summary>

                        <div className="mt-4 md:mt-6 space-y-4 md:space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
                            <div className="space-y-1">
                                <Label className="text-[8px] uppercase tracking-widest text-primary/50 ml-1">Initialization Vector (IV)</Label>
                                <div className="p-3 bg-black/40 border border-white/5 rounded-sm font-mono text-[8px] md:text-[9px] break-all text-amber-500/70 shadow-inner">
                                    {item.rawIv}
                                </div>
                            </div>
                            <div className="space-y-1">
                                <Label className="text-[8px] uppercase tracking-widest text-primary/50 ml-1">Encrypted Payload (AES-256-GCM)</Label>
                                <div className="p-3 bg-black/40 border border-white/5 rounded-sm font-mono text-[8px] md:text-[9px] break-all text-blue-500/70 max-h-24 md:max-h-32 overflow-auto scrollbar-hide shadow-inner">
                                    {item.rawEnc}
                                </div>
                            </div>
                        </div>
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between pt-6 md:pt-8 mt-6 border-t border-white/10 relative z-20 gap-4">
                    <div className="flex space-x-3">
                        <button
                            onClick={handleDelete}
                            className="flex-1 sm:flex-none flex items-center justify-center space-x-2 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-red-500 hover:bg-red-500/10 border border-red-500/20 rounded-sm transition-all shadow-[0_0_15px_rgba(239,68,68,0.1)]"
                        >
                        ...
                        <button
                            onClick={onEdit}
                            className="flex-1 sm:flex-none px-6 py-2 text-[10px] font-black uppercase tracking-widest text-primary/60 hover:text-primary hover:bg-white/5 border border-white/10 rounded-sm transition-all text-center"
                        >
                            Re-Forge
                        </button>
                    </div>
                    <Button
                        onClick={onClose}
                        className="bg-primary text-black text-[10px] font-black uppercase tracking-widest px-8 h-10 rounded-sm shadow-[0_0_20px_rgba(255,176,0,0.2)] w-full sm:w-auto"
                    >
                        Secure Close
                    </Button>
                </div>
