...
export default function Dashboard() {
    ...
    return (
        <div className="flex h-screen bg-background overflow-hidden font-sans relative selection:bg-primary/30 selection:text-primary">
            ...
            {/* Sidebar */}
            <aside className="w-64 border-r border-primary/20 bg-black/80 backdrop-blur-md flex flex-col p-4 space-y-4 shadow-[5px_0_30px_rgba(0,0,0,0.5)] z-20 relative overflow-hidden">
                {/* Sidebar Edge Glow */}
                ...
                <div className="px-2 pb-4 border-b border-white/5 mb-2">
                    <div className="flex items-center space-x-2 text-[10px] uppercase tracking-widest text-muted-foreground bg-white/5 p-2 rounded-sm border border-white/5">
            ...
            <main className="flex-1 flex flex-col overflow-hidden relative z-10">
                ...
                {view === 'settings' ? (
                    ...
                    <div className="flex-1 overflow-auto p-8 scrollbar-hide relative group/content">
                        <div className="flex items-center justify-between mb-8 border-l-2 border-primary/50 pl-4 py-1 bg-primary/5">
                            <div>
                                <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-primary/70 mb-1">Secure Directory</h2>
                                <h1 className="text-3xl font-black uppercase tracking-tighter text-white">
                                    {activeTab === 'all' ? 'Master Vault' : activeTab + ' Archives'}
                                </h1>
                            </div>
                            <div className="text-right">
                                <div className="text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Record Count</div>
                                <span className="font-mono text-xl text-primary">{filteredItems.length.toString().padStart(2, '0')}</span>
                            </div>
                        </div>

                        {filteredItems.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-96 border border-white/5 rounded-sm bg-black/40 backdrop-blur-sm relative overflow-hidden group">
                                ...
                                <p className="text-xs uppercase tracking-[0.3em] font-bold text-muted-foreground">No Records Found</p>
                                {search ? <p className="text-[10px] uppercase opacity-40 mt-2">Adjust search parameters</p> : <p className="text-[10px] uppercase opacity-40 mt-2">Initialize directory by adding a record</p>}
                            </div>
                        ) : (
                                    ...
                                    >
                                        ...
                                        <div className="flex justify-between items-start mb-6">
                                            <div className={cn(
                                                ...
                                                {item.type === 'login' && <User className="w-5 h-5" />}
                                                {item.type === 'card' && <CreditCard className="w-5 h-5" />}
                                                {item.type === 'note' && <FileText className="w-5 h-5" />}
                                                {item.type === 'id' && <Shield className="w-5 h-5" />}
                                            </div>
                                            <span className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest bg-white/5 px-2 py-0.5 rounded-sm">
                                                {new Date(item.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: '2-digit' })}
                                            </span>
                                        </div>

                                        <h3 className="font-bold text-sm uppercase tracking-wider truncate mb-1 group-hover:text-primary transition-colors">{item.data.name || "UNNAMED RECORD"}</h3>
                                        <p className="text-[10px] font-mono text-muted-foreground truncate opacity-60 tracking-tight">
                                        ...
                                        <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between">
                                            ...
                                            <span className="text-[8px] font-bold text-primary/40 uppercase tracking-widest">Encr Mode: AES-GCM</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
